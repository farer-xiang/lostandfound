							...
							<div class="li">
								<table class="table table-bordered">
								  ...
								  <tbody id="listDivId">
									...
									<tr v-for="(foundInfo,index) in foundInfos" :key="foundInfo.fid">
								      <td>{{foundInfo.fname}}</td>
								      <td>{{dateInfos[index]}}</td>
								      <td>{{foundInfo.fstatus|status}}</td>
								      <td class="op">
									      <a @click="detail(foundInfo.fid)" style="cursor: pointer">查看</a>
									      &nbsp;&nbsp;
									      <div style="display:none;"><a id="clickmodal" href="#modal-container-606893" data-toggle="modal"></a></div>
									      <a @click="del(foundInfo.fid)" style="cursor: pointer">删除</a>
									      
								      </td>
								    </tr>
									<tr v-if="foundInfos.length === 0">
										...
										<div class="modal-dialog" style="width:1155px">
											<div class="modal-content">
												...
												<div class="table_container" id="infotable">
												  	<div class="table_img">
													  		<img :src="'img/faces/'+fimage" alt="picture" />
													  </div>
	...
	var vm1 = new Vue({
		...
		methods:{
				...
				$.ajax({
			        ...
			        success: function (json) {
			            if(json.found!=""){
			                ...
			                vm2.fname=json.found.fname;
			                vm2.fdate=json.time;
			                vm2.fpos=json.found.fpos;
			                vm2.tel=json.user.tel;
			                vm2.email=json.user.email;
			                vm2.fimage=json.found.fimage;
			...
			del:function(message){
				...
				if(confirm("确认删除？"))
					window.location.href='admin/delfound/'+message;
				else
					return false;
			}
		},
